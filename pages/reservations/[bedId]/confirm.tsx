...
async function confirmPage(user: User, bedid: number) {
  ...
  return (
    <div>
      ...
      <form method="post">
        <a href="/reservations" className="btn">Späť</a>
        <button type="submit" className="btn">Rezervovať ({cost})</button>
      </form>
    </div>
  )
}
...
export async function get(req: SessionRequest) {
  return renderPage(await confirmPage(req.session!.user, parseInt(req.params.bedId)), req)
}
