...
async function Page(req: SessionRequest) {
  ...
  if (req.parsedUrl.searchParams.has("tag") && req.parsedUrl.searchParams.get("tag") != "any") {
    items = items.filter(e => e.item!.tags.some(e => e.name == req.parsedUrl.searchParams.get("tag")))
  }
  ...
  return <div>
    ...
    <form method="get">
      <select name="tag" value={req.parsedUrl.searchParams.get("tag") ?? "any"}>
        <option value="any">Všetky</option>
        {tags.map(e => <option value={e.name}>{e.name}</option>)}
      </select>
      <button type="submit" className="btn">Filtrovať</button>
    </form>
    <table>
      ...
      {items.map(e => {
        return <tr className="border-t-gray-200 border-t border">
          ...
          <td className="p-2">
            <form method="post">
              ...
              <button type="submit" className="btn">Vybaviť</button>
            </form>
          </td>
        </tr>
      })}
