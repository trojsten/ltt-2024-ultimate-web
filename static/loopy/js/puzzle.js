...
var Puzzle = (function ($) {
  ...
  function updateControlState() {
    ...
    if (status > 0) {
      if (!showedSolution) {
        ...
        console.log((Date.now() - gameStart) / 1000)
        window.fakeStorage.setItem('result', JSON.stringify({ score: (Date.now() - gameStart) / 1000, data: { preset } }))
      }
    } else if (status < 0) {
  ...
  function frontend_default_colour(/* float* */ colourptr) {
    ...
    setValue(colourptr, rgbFloat[0], type)
    colourptr += size
    setValue(colourptr, rgbFloat[1], type)
    colourptr += size
  ...
  function calcMaxCanvasSize() {
    ...
    if (!options.hasStatusBar && $status.is(':visible')) {
      // e.g., during initial load
  ...
  var animationId, lastAnimationTime
  ...
  function initCImports() {
    ...
    midend_restart_game = Module.cwrap('midend_restart_game', 'void', [
      'number'
    ])
    midend_set_params = Module.cwrap('midend_set_params', 'void', [
      'number',
      'number'
    ])
    midend_get_params = Module.cwrap('midend_get_params', 'number', ['number'])
    midend_size = Module.cwrap('midend_size', 'void', [
      'number',
      'number',
      'number',
      'number'
    ...
    midend_force_redraw = Module.cwrap('midend_force_redraw', 'void', [
      'number'
    ])
    midend_process_key = Module.cwrap('midend_process_key', 'number', [
      'number',
      'number',
      'number',
      'number'
    ...
    midend_which_preset = Module.cwrap('midend_which_preset', 'number', [
      'number'
    ])
    midend_wants_statusbar = Module.cwrap('midend_wants_statusbar', 'number', [
      'number'
    ])
